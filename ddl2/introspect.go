...
func (dbi *DatabaseIntrospector) GetExtensions(ctx context.Context, filter *Filter) (extensions []string, err error) {
	...
	rows, err := dbi.queryContext(ctx, srcDir, "sql/postgres_extensions.sql", filter)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (dbi *DatabaseIntrospector) GetTables(ctx context.Context, filter *Filter) ([]Table, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_tables.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		// TODO: convert this to use pg_catalog (because faster)
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_tables.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_tables.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetColumns(ctx context.Context, filter *Filter) ([]Column, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_columns.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		// TODO: convert this to use pg_catalog (because faster)
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_columns.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_columns.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetConstraints(ctx context.Context, filter *Filter) ([]Constraint, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_constraints.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		// TODO: convert this to use pg_catalog (because faster)
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_constraints.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_constraints.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetIndexes(ctx context.Context, filter *Filter) ([]Index, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetTriggers(ctx context.Context, filter *Filter) ([]Trigger, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetFunctions(ctx context.Context, filter *Filter) ([]Function, error) {
	...
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_functions.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
...
func (dbi *DatabaseIntrospector) GetViews(ctx context.Context, filter *Filter) ([]View, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/sqlite_views.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/postgres_views.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, srcDir, "sql/mysql_views.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
