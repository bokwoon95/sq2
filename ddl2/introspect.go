...
func (dbi *DatabaseIntrospector) GetTables(ctx context.Context, filter *Filter) ([]Table, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_tables.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
	...
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_tables.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetColumns(ctx context.Context, filter *Filter) ([]Column, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_columns.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
	...
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_columns.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetConstraints(ctx context.Context, filter *Filter) ([]Constraint, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_constraints.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
	...
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_constraints.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetIndexes(ctx context.Context, filter *Filter) ([]Index, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/postgres_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_indexes.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetTriggers(ctx context.Context, filter *Filter) ([]Trigger, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/postgres_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_triggers.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
...
func (dbi *DatabaseIntrospector) GetViews(ctx context.Context, filter *Filter) ([]View, error) {
	...
	switch dbi.dialect {
	case sq.DialectSQLite:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/sqlite_views.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectPostgres:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/postgres_views.sql", filter)
		if err != nil {
			return nil, err
		}
	case sq.DialectMySQL:
		rows, err = dbi.queryContext(ctx, dataDir, "sql/mysql_views.sql", filter)
		if err != nil {
			return nil, err
		}
	default:
