...
func TestSubquery(t *testing.T) {
	...
	t.Run("subquery query no fields", func(t *testing.T) {
		t.Parallel()
		var tt TT
		tt.item = SQLite(nil).From(NewSubquery("subquery", MySQL(nil).Select())).Select(Literal("*"))
		_, _, _, err := ToSQL("", tt.item)
		if err == nil {
			t.Fatal(testutil.Callers(), "expected error but got nil")
		}
	...
	t.Run("subquery query field no name", func(t *testing.T) {
		t.Parallel()
		var tt TT
		tt.item = SQLite(nil).From(NewSubquery("subquery", MySQL(nil).Select(Value(1)))).Select(Literal("*"))
		_, _, _, err := ToSQL("", tt.item)
		if err == nil {
			t.Fatal(testutil.Callers(), "expected error but got nil")
		}
	...
	t.Run("subquery query faulty sql", func(t *testing.T) {
		...
		var tt TT
		tt.item = SQLite(nil).
			From(NewSubquery("subquery", MySQL(nil).Select(Value(1).As("field")).Where(FaultySQL{}))).
			Select(Literal("*"))
		_, _, _, err := ToSQL("", tt.item)
...
func Test_SubqueryField(t *testing.T) {
	...
	t.Run("Subquery field not exists", func(t *testing.T) {
		t.Parallel()
		var tt TT
		q := NewSubquery("subquery", MySQL(nil).Select(Value(1).As("field")))
