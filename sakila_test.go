...
func (tbl ACTOR) DDL(dialect string, t *ddl.T) {
...
func (tbl CATEGORY) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl COUNTRY) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl CITY) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl ADDRESS) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl LANGUAGE) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl FILM) DDL(dialect string, t *ddl.T) {
...
func (tbl FILM_ACTOR) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl FILM_ACTOR_REVIEW) DDL(dialect string, t *ddl.T) {
	FILM_ACTOR := NEW_FILM_ACTOR("")
	t.PrimaryKey(tbl.FILM_ID, tbl.ACTOR_ID)
	t.ForeignKey(tbl.FILM_ID, tbl.ACTOR_ID).References(FILM_ACTOR, FILM_ACTOR.FILM_ID, FILM_ACTOR.ACTOR_ID).OnUpdate(ddl.CASCADE).OnDelete(ddl.RESTRICT)
...
func (tbl FILM_CATEGORY) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl STAFF) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl STORE) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl CUSTOMER) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl INVENTORY) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
...
func (tbl RENTAL) DDL(dialect string, t *ddl.T) {
	if dialect == sq.DialectSQLite {
		t.Trigger(sqliteLastUpdateTriggerFmt, sq.Literal("rental_last_update_after_update_trg"), tbl)
	}
	if dialect == sq.DialectPostgres {
		t.NameExclude("rental_range_excl", "GIST", ddl.Exclusions{
			{tbl.INVENTORY_ID, "="},
...
func (view ACTOR_INFO) DDL(dialect string, v *ddl.V) {
	ACTOR := NEW_ACTOR("a")
	FILM := NEW_FILM("f")
...
func (view CUSTOMER_LIST) DDL(dialect string, v *ddl.V) {
...
func (view FILM_LIST) DDL(dialect string, v *ddl.V) {
...
func (view NICER_BUT_SLOWER_FILM_LIST) DDL(dialect string, v *ddl.V) {
	CATEGORY := NEW_CATEGORY("")
	FILM_CATEGORY := NEW_FILM_CATEGORY("")
	FILM := NEW_FILM("")
	FILM_ACTOR := NEW_FILM_ACTOR("")
...
func (view SALES_BY_FILM_CATEGORY) DDL(dialect string, v *ddl.V) {
...
func (view SALES_BY_STORE) DDL(dialect string, v *ddl.V) {
	PAYMENT := NEW_PAYMENT("p")
	RENTAL := NEW_RENTAL("r")
	INVENTORY := NEW_INVENTORY("i")
	STORE := NEW_STORE("s")
...
func (view STAFF_LIST) DDL(dialect string, v *ddl.V) {
	STAFF := NEW_STAFF("s")
	ADDRESS := NEW_ADDRESS("a")
	CITY := NEW_CITY("ci")
	COUNTRY := NEW_COUNTRY("co")
...
func (view FULL_ADDRESS) DDL(dialect string, v *ddl.V) {
	const triggerFmt = `
